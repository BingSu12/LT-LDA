function [template,alignpath,sigmaw] = TemplateClustering(classnum,trainset,trainsetnum,max_iteration_num_ini,template_length,band_factor)
        ...
        for i = 1:trainsetnum(c)
            ...
            for temp_align_count = 1:template_length
                ...
                for temp_index_count = temp_start:temp_end
                    temp_vector = features(temp_index_count,:) - template{c}(temp_align_count,:);
                    state_variance{c}{temp_align_count} = state_variance{c}{temp_align_count} + temp_vector'*temp_vector;
                end
            end
        end
        for temp_align_count = 1:template_length
            if template_num{c}(temp_align_count)~=0
                state_variance{c}{temp_align_count} = state_variance{c}{temp_align_count}./template_num{c}(temp_align_count);
            end
        end
    end
