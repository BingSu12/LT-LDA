function Evaluate_LTLDA_time()
    ...
    for c=1:classnum
        for m = 1:trainsetnum(c)
            trainset{c}{m} = bsxfun(@minus, trainset{c}{m}, traindatamean);
        end
    end
    for m = 1:testsetdatanum
        testsetdata{m} = bsxfun(@minus, testsetdata{m}, traindatamean);
    end
    ...
    %% calculating the LT-LDA projection

    [template_ini,alignpath_ini,sigmaw] = TemplateClustering(classnum,trainset,trainsetnum,max_iteration_num_ini,template_length,band_factor);
    ini_sigmaw = sigmaw;
    
    transMatrix_ini = getTransChange(ini_sigmaw,ini_sigmat,downdim);
    [transMatrix,template,alignpath,sigmaw] = getICMLTrans_LTLDA(sigmat,transMatrix_ini,trainset,trainsetnum,classnum,downdim,max_iteration_num,template_length,band_factor);
            
    traintime = toc;
               
    %% classfication in the subspace  
    traindownset = cell(charnum,1);
    testdownsetdata = cell(testsetdatanum,1);

    for j = 1:charnum
        traindownset{j} = cell(trainsetnum(j),1);
        for m = 1:trainsetnum(j)
            %traindownset{j}{m} = bsxfun(@plus, trainset{j}{m} * transMatrix, traindatamean * transMatrix);
            traindownset{j}{m} = trainset{j}{m} * transMatrix;
        end
    end
    for j = 1:testsetdatanum
        %testdownsetdata{j} = bsxfun(@plus, testsetdata{j} * transMatrix, traindatamean * transMatrix);
        testdownsetdata{j} = testsetdata{j} * transMatrix;
    end
    
    ALL_Data_mbh = cell(1,charnum);
    for c = 1:charnum
        temp_all_data_mbh = zeros(trainsetnum(c),2*5*downdim);
        for sample_count = 1:trainsetnum(c)
            W = genRepresentation(traindownset{c}{sample_count},CVAL);
            temp_all_data_mbh(sample_count,:) = W';
        end
        ALL_Data_mbh{c} = temp_all_data_mbh;
    end

    ALL_Data_mbh_test = zeros(testsetdatanum,2*5*downdim);
    for sample_count = 1:testsetdatanum
        W = genRepresentation(testdownsetdata{sample_count},CVAL);
   ...
   [~,TestClass] = max(TestClass_ALL,[],2);   		
    ...
    for wi = 1 : size(weights,1)
        ...
        TestData_Kern = zeros(size(TestData_Kern_cell{1}));
            ...
            [precision(wi,:),recall(wi,:),acc(wi) ] = train_and_classify3(TrainData_Kern,TestData_Kern,TrainClass,TestClass);       
    end          
    ...
    LTLDA_svm_down_acc_best = max(acc);
    LTLDA_svm_down_recall = mean(recall);
    LTLDA_svm_down_precision = mean(precision);
    LTLDA_svm_down_F = mean(F);
            
    fprintf('Training time of LT-LDA is %.4f \n',traintime);
    fprintf('Classification using rank pooling and SVM:\n');
    fprintf('MAP is %.4f \n',LTLDA_svm_down_map);
    fprintf('Accuracy is %.4f \n',LTLDA_svm_down_reg);
    fprintf('Precision is %.4f \n',LTLDA_svm_down_precision);
    fprintf('Recall is %.4f \n',LTLDA_svm_down_recall);
    fprintf('F-score is %.4f \n',LTLDA_svm_down_F);
end
...
function [precision,recall,acc ] = train_and_classify3(TrainData_Kern,TestData_Kern,TrainClass,TestClass)
         nTrain = 1 : size(TrainData_Kern,1);
         TrainData_Kern = [nTrain' TrainData_Kern];         
         nTest = 1 : size(TestData_Kern,1);
         TestData_Kern = [nTest' TestData_Kern];         
